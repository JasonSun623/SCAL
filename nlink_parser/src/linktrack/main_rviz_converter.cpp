...
#include <ros/ros.h>
ros::Publisher pub_pose,path_pub;
nav_msgs::Path  path;
...
void Anchorframe0Callback(const nlink_parser::LinktrackAnchorframe0 &msg)
{
  ...
  for (const auto &node : msg.nodes)
  {
    ...
    if (!poses.count(id))
    {
      ...
      output_pose.header.frame_id = frameId;
      output_pose.pose.pose.orientation.w = 0;
      output_pose.pose.pose.orientation.x = 0;
      output_pose.pose.pose.orientation.y = 0;
      output_pose.pose.pose.orientation.z = 1;
    }
    auto &msg_pose = poses[id].msg;
    ++msg_pose.header.seq;
     // ros::Time(msg.system_time / 1000.0)
    output_pose.header.stamp = ros::Time::now();
    output_pose.pose.pose.position.x = static_cast<double>(node.pos_3d[0]);
    output_pose.pose.pose.position.y = static_cast<double>(node.pos_3d[1]);
    output_pose.pose.pose.position.z = static_cast<double>(node.pos_3d[2]);
    poses.at(id).publish();
...
void Tagframe0Callback(const nlink_parser::LinktrackTagframe0 &msg)
{
  ...
  ++msg_pose.header.seq;
  msg_pose.header.stamp = ros::Time::now(); // ros::Time(msg.system_time / 1000.0)
