...
namespace linktrack
{
  ...
  Init::Init(NProtocolExtracter *protocol_extraction, serial::Serial *serial)
  {
    ...
    // Smooth_initNodeFrame2(protocol_extraction);
  ...
  // void Init::Smooth_initNodeFrame2(NProtocolExtracter *protocol_extraction)
  // {
  //   auto protocol = new NLT_ProtocolNodeFrame2;
  //   protocol_extraction->AddProtocol(protocol);
  //   protocol->SetHandleDataCallback(
  ...
  //         if (!publishers_[protocol])
  //         {
  //           auto topic = "nlink_linktrack_nodeframe2_smoothed";
  //           publishers_[protocol] =
  //               nh_.advertise<nlink_parser::LinktrackNodeframe2>(topic, 200);
  //           TopicAdvertisedTip(topic);
  //         }
  ...
  //         // RawRanges[i].push_back(msg.nodes[i].dis);
  //         // msg.nodes[i].dis = RawRanges[i].smoothRange();
  //         // smoothed_range[i] = RawRanges[i].smoothRange();
  //         // msg.nodes[i].dis = smoothed_range[i];

  //         msg_nodes.resize(data.valid_node_count);
  //         for (size_t i = 0; i < data.valid_node_count; ++i)
  //         {
  //         RawRanges[i].push_back(data.nodes[i].dis);
  //         data.nodes[i].dis = RawRanges[i].smoothRange();
  //           auto &msg_node = msg_nodes[i];
  //           auto node = data.nodes[i];
  //           msg_node.id = node->id;
  //           msg_node.role = node->role;
